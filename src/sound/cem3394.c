...
{
	const struct cem3394_interface *intf = (const struct cem3394_interface *)msound->sound_interface;
	int i;
	...
	mixer_buffer = (INT16*)malloc(2 * sample_rate * sizeof(INT16));
	if (!mixer_buffer)
		return 1;
...
static INLINE float compute_db(float voltage)
{
...
static INLINE UINT32 compute_db_volume(float voltage)
{
	float temp;
